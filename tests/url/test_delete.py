...
async def test_delete_slug(client: AsyncClient, auth_tokens: str) -> None:
    ...
    delete_resp = await client.delete(
        f"/url/{slug}", headers={"Authorization": f"Bearer {auth_tokens}"}
    )

    assert delete_resp.status_code == status.HTTP_204_NO_CONTENT


async def test_delete_slug_unauthorize(client: AsyncClient, auth_tokens: str) -> None:
    create_resp = await client.post(
        "/url",
        json={"original_url": "https://google.com"},
        headers={"Authorization": f"Bearer {auth_tokens}"},
...
async def test_delete_slug_permission_denied(
    ...
    delete_resp = await client.delete(
        f"/url/{slug}", headers={"Authorization": f"Bearer {auth_tokens2}"}
    )
